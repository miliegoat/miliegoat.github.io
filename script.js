  ...
  function updateProfile(data) {
    ...
    document.getElementById('discordProfile').innerHTML = `
      ...
      <div class="profile-info">
        <div class="profile-name">${displayName} <span class="star">☆</span></div>
    ...
    if (listening_to_spotify && spotify) {
          ...
          <div class="activity-inner">
            ...
            <div class="activity-info">
              <div class="activity-type" style="display:flex;justify-content:space-between;align-items:center;">
                ...
                <button class="lyrics-btn" id="lyricsBtn" onclick="toggleLyrics()" title="show lyrics">
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor"><path d="M3 18h12v-2H3v2zm0-5h12v-2H3v2zm0-7v2h12V6H3zm13 9.59L19.59 12 16 8.41 17.41 7l5 5-5 5L16 15.59z"/></svg>
                  lyrics
                </button>
              </div>
      ...
      if (trackChanged) {
        ...
        panel.style.opacity = lyricsOpen ? '0' : '1';
      ...
      if (lyricsOpen) {
        lyricsOpen = false;
        const lw = document.getElementById('lyricsWindow');
  ...
  function highlightLyric(elapsedMs) {
    ...
    document.querySelectorAll('.lyric-line').forEach((el, i) => {
      el.classList.toggle('lyric-active', i === active);
      el.classList.toggle('lyric-past', i < active);
    });
    const activeEl = document.getElementById(`lyric-${active}`);
    const content = document.getElementById('lyricsContent');
    const panel = document.getElementById('lyricsPanel');
    if (activeEl && content && panel) {
      const panelH = panel.clientHeight;
      const elTop = activeEl.offsetTop - 0;
      const elH = activeEl.clientHeight;
      const shift = -elTop + (panelH / 2) - (elH / 2);
      content.style.transform = `translateY(${shift}px)`;
    }
    highlightLyricsWindow(active);
  }

  function closeLyricsWindow() {
    const win = document.getElementById('lyricsWindow');
    if (!win) return;
    win.style.animation = 'lw-pop-in 0.25s cubic-bezier(0.36, 0, 0.66, -0.56) forwards';
    const panel = document.getElementById('lyricsPanel');
    if (panel) {
      panel.style.display = 'block';
  ...
  function openLyricsWindow() {
    ...
    if (document.getElementById('lyricsWindow')) {
      const existing = document.getElementById('lyricsWindow');
      existing.style.animation = 'none';
      existing.offsetHeight;
      existing.style.animation = 'lw-pop-out 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) forwards';
      existing.style.display = 'flex';
      updateLyricsWindow();
      if (currentSpotifyData && currentLyrics.length) {
        const elapsed = Date.now() - currentSpotifyData.timestamps.start;
        highlightLyricsWindow(currentLyrics.reduce((acc, l, i) => elapsed >= l.ms ? i : acc, 0));
      }
      return;
    }
    ...
    win.id = 'lyricsWindow';
    win.innerHTML = `
      <div class="lw-titlebar" id="lwTitlebar">
        <div class="lw-title-info">
          ...
          <span id="lwSong">lyrics</span>
        </div>
        <div class="lw-controls">
          <button class="lw-btn lw-close" id="lwClose" title="close">✕</button>
        </div>
      </div>
    ...
    win.style.left = Math.max(20, (window.innerWidth - 500) / 2) + 'px';
    win.style.top = Math.max(20, (window.innerHeight - 520) / 2) + 'px';

    const titlebar = document.getElementById('lwTitlebar');
    ...
    document.getElementById('lwClose').addEventListener('click', closeLyricsWindow);
  ...
  function updateLyricsWindow() {
    ...
    if (currentLyrics.length > 0) {
      ...
      lyricsEl.innerHTML = document.getElementById('lyricsContent').innerHTML
        .replace(/lyric-line/g, 'lw-lyric-line')
        .replace(/id="lyric-/g, 'id="wlyric-');
    }
  }

  function highlightLyricsWindow(active) {
    const win = document.getElementById('lyricsWindow');
    if (!win || win.style.display === 'none') return;
    const lyricsEl = document.getElementById('lwLyrics');
    if (!lyricsEl) return;
    document.querySelectorAll('.lw-lyric-line').forEach((el, i) => {
