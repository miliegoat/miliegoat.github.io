import { currentSpotifyData, currentLyrics, setCurrentLyrics } from './state.js';
...
function parseLrc(lrc) {
  return lrc.split('\n')
    .map(line => {
      ...
      if (!m) return null;
      return { ms: (parseInt(m[1], 10) * 60 + parseFloat(m[2])) * 1000, text: m[3].trim() };
    })
...
function renderLyricsInPanel() {
  ...
  if (currentLyrics.length > 0) {
    lyricsEl.innerHTML = currentLyrics.map(function(l, i) {
      return '<div class="lw-lyric-line" id="wlyric-' + i + '">' + l.text + '</div>';
    }).join('');
...
export function highlightLyric(elapsedMs) {
  ...
  if (activeEl) {
    var target = activeEl.offsetTop - (lyricsEl.clientHeight / 2) + (activeEl.clientHeight / 2);
...
export function openLyricsPanel() {
  ...
  if (currentSpotifyData && currentLyrics.length) {
    setTimeout(function() { highlightLyric(Date.now() - currentSpotifyData.timestamps.start); }, 150);
  }
}
...
export function closeLyricsPanel() {
  ...
  setTimeout(function() { panel.remove(); }, 390);
}
...
export function fetchLyrics(spotify) {
  ...
  try {
    ...
    var duration = Math.round((spotify.timestamps.end - spotify.timestamps.start) / 1000);
    var url = 'https://lrclib.net/api/get?artist_name=' + encodeURIComponent(artist) + '&track_name=' + encodeURIComponent(spotify.song) + '&duration=' + duration;
    fetch(url)
...
export function toggleLyrics() {
  var lyricsOpen = !document.getElementById('lyricsPanel') || document.getElementById('lyricsPanel').style.display === 'none';
  if (lyricsOpen) {
