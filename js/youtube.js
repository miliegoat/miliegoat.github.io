...
import {
  setYtPlayer, setYtReady, setYtPendingVideo, setYtCurrentVideoId,
  setYtUnlocked, ytSearchCache, ytPlayer, ytReady, ytPendingVideo,
  ytCurrentVideoId, ytUnlocked,
} from './state.js';
...
function createYTPlayer(videoId, seekSec) {
  ...
  var player = new YT.Player('yt-iframe', {
    ...
    events: {
      ...
      onError: function() { updatePlayAlongBtn('error'); },
    },
  });
  setYtPlayer(player);
}
...
async function searchYouTube(spotify) {
  ...
  for (var i = 0; i < queries.length; i++) {
    var query = queries[i];
    try {
      var url = 'https://www.googleapis.com/youtube/v3/search?part=snippet&q=' + encodeURIComponent(query) + '&type=video&maxResults=1&key=' + YT_API_KEY;
      ...
      if (!res.ok) continue;
      var videoId = data.items && data.items[0] && data.items[0].id ? data.items[0].id.videoId : null;
      if (videoId) {
