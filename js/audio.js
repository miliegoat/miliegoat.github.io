...
export function initAudioPlayer() {
  ...
  function setVolume(v) {
    ...
    if (volSlider) {
      ...
      volSlider.style.background = 'linear-gradient(to right, ' + accent + ' ' + (v * 100) + '%, ' + border + ' ' + (v * 100) + '%)';
    }
  }
  ...
  audio.addEventListener('timeupdate', () => {
    ...
    if (currentTimeEl) currentTimeEl.textContent = fmtSeconds(audio.currentTime);
  });
  ...
  audio.addEventListener('loadedmetadata', () => {
    ...
    if (volSlider) {
      ...
      const accent = computedStyle.getPropertyValue('--accent').trim();
      const border = computedStyle.getPropertyValue('--border').trim();
      volSlider.style.background = 'linear-gradient(to right, ' + accent + ' ' + (volSlider.value * 100) + '%, ' + border + ' ' + (volSlider.value * 100) + '%)';
    }
  });
  ...
  if (volSlider) {
    ...
    volSlider.style.background = 'linear-gradient(to right, ' + initAccent + ' 100%, ' + initBorder + ' 100%)';
  }
  if (audio.duration) {
    const durationEl = document.getElementById('durationEl');
    if (durationEl) durationEl.textContent = fmtSeconds(audio.duration);
